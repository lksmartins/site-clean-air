                ...
                {state.map(item=>{
                    return (
                        <div key={item.id} className={`${styles.inputGroup} ${item.type == 'checkbox' ? styles.inline : null}`}>
                            ...
                            <input id={`formId_${item.id}`} name={item.id} disabled={item.disabled && item.disabled} type={item.type}  onChange={handleInputChange}  accept={item.type=="file"?"image/*, .pdf, .doc, .docx":"*" }/>
                        </div>
                    )
                })}
